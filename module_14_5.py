...
async def get_buying_list(message):
    ...
    for i in range(0, 4):
        await message.answer(f"Название: {crud[i][1]} | Описание: {crud[i][2]} | Цена: {crud[i][3]}")
...
async def set_username(message, state):
    ...
    hold = await state.get_data()

    check = is_included(hold['username'])
    if check is True:
        await state.update_data(username=message.text)
...
async def set_age(message, state):
    await state.update_data(age=message.text)
    db_transfer = await state.get_data()
    add_user(db_transfer['username'], db_transfer['email'], db_transfer['age'])
